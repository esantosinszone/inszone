 ...
 * classes. Elements opt in with `.reveal-up`, `.reveal-left` or `.reveal-fade`
 * and get `--visible` added once they enter the viewport.
 *
...
export type RevealEffect = 'up' | 'left' | 'fade' | 'draw'
...
export const initReveal = ( root: ParentNode = document ): void => {
  ...
  if ( prefersReducedMotion() ) {
    ( [ 'up', 'left', 'fade', 'draw' ] as const ).forEach(( effect ) => {
      root
  ...
  instances = ( [ 'up', 'left', 'fade', 'draw' ] as const ).map(( effect ) => {
